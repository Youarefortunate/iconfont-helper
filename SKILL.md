...
1. 先判断用户要操作的项目 key，优先使用 [iconfont.json](./scripts/iconfont.json) 中已注册的 `--project`。
...
| :--- | :--- | :--- | :--- |
| `demo-mp` | 示例小程序 | `1234567` | `demo-icons` |
| `component-lib` | 示例组件库 | `7654321` | `component-icons` |

项目的所有输出文件映射关系（如本地 SVG 目录、编译输出路径等）统一管理在 [iconfont.json](./scripts/iconfont.json) 配置文件中。发布版默认只保留示例配置；实际使用时可复制 [iconfont.example.json](./scripts/iconfont.example.json) 为 `iconfont.json` 并替换为自己的项目参数。各项目可独立定义所需参数及输出规则。
...
| `font_name` | `string` | 是 | 生成的字体族名称，例如 `demo-icons`。 |
| `associated_project` | `string` | 否 | 关联小程序项目路径，例如 `apps/demo-mp`。 |
| `svg_dir` | `string` | 是 | 本地存放同步 SVG 文件的路径。 |
